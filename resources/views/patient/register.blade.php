<x-guest-layout>
    <h4 class="text-red-600 text-center">Vaccine Registration System</h4>
    
    @if (session('success'))
        <div id="toast-success"
            class="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800"
            x-data="{ show: true }" class="dark:text-%s-400" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
            role="alert">
            <div
...
<div id="toast-warning" class="flex items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
    ...
    <form method="POST" action="{{ route('patient.register') }}">
        ...
        <div class="mt-4">
            ...
            <select class="block mt-1 w-full rounded-lg border-grey-100" id="vaccine_center_id"
                class="block mt-1 w-full" name="vaccine_center_id" required>
                <option class="block mt-1 w-full ">Select vaccine center</option>
                @foreach ($vaccineCenters as $center)
                    <option class="block mt-1 w-full " value="{{ $center['id'] }} "@selected(old('vaccine_center_id') === $center['id'])>
                        {{ $center['name'] }}</option>
                @endforeach
